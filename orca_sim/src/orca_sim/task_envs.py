...
class OrcaHandRightCubeOrientation(BaseOrcaHandEnv):
    ...
    def _sustained_goal_reached(self) -> bool:
        return self._success_counter >= self.success_steps
    ...
    def _get_reward(self):
        if self._sustained_goal_reached():
            return 50

        if self._cube_dropped():
            return -2.0

        alignment = self._red_face_up_alignment()  # [-1, 1]
        alignment_reward = 0.1 * (alignment + 1.0)

        z = self.data.xpos[self._cube_body_id, 2]
        lift_reward = np.tanh(5.0 * (z - 0.08))

        timeout_penalty = -0.01

        return alignment_reward + 0.2 * lift_reward + timeout_penalty

    def _get_terminated(self) -> bool:
        return (
            self._sustained_goal_reached()
            or self._cube_dropped()
        )
    ...
    def _get_info(self) -> dict[str, Any]:
        info = {
            ...
            "is_success": self._sustained_goal_reached(),
        ...
        elif self._cube_dropped():
            info["term_reason"] = "dropped"
